		...
		posting.done(function( data ){
			...
			if(data == 0){
				...
				setTimeout(function(){
					...
					$('#interface ul').append('<li><div class="columns sixteen"> <p>Welcome: please an adviser will be with you shortly...</p> </div></li>');
					
				}, 5000);
			
		 		chat2 = setInterval(function(){
...
/********************************************************UPDATE****************************************/
	...
	$( "#updateTables" ).change(function() {
	  ...
	  str = $( "#updateTables option:selected" ).val();
	  //alert(str);
	  ...
	  $("#updateRows").change(function(){
		...
		$.getJSON(url2, function(data2){
	  		...
	  		$("#dbColumns").change(function(){
	  			...
	  			id = $( "#updateRows option:selected" ).attr("id");
	  			col = $( "#dbColumns option:selected" ).val();
	  			var table = str;
	  			//console.log(id);
...
/*************************************FEEDBACK FORM****************************************/
	...
	$('#feedbackForm').submit(function( event ){
		...
		senderEmail = $form.find( "input[name='email']" ).val(),
		subject = $form.find( "input[name='subject']" ).val(),
		message = $form.find( "textarea[name='message']" ).val(),
		url = $form.attr( "action" );
		
		var posting = $.post( url, { senderName: senderName, senderEmail: senderEmail, subject: subject, message: message } );
